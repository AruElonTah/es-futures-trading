...
2. **BL-1 lookahead-detector CI gate**: the test `tests/integration/test_lookahead.py` constructs a deliberately-leaking ORB variant (`close.shift(-1)`-based entry), routes it through `safe_from_signals()`, asserts the resulting Sharpe is finite (not infinite) and `win_rate <= 0.90`; the test runs in the PR CI workflow and is required to pass for merge. *(Note: original SC specified 40–60% band; accepted deviation — flat fixture is degenerate with no real breakout, so 40–60% is not meaningful. Threshold relaxed to ≤ 90% by operator decision 2026-05-20.)*
...
- **BL-1 lookahead-leakage detector gate** — Phase 3 must not exit until the `tests/integration/test_lookahead.py` CI test is green: a deliberately-leaking ORB variant routed through `safe_from_signals()` must produce a finite Sharpe and `win_rate <= 0.90`. *(Win rate threshold relaxed from 40–60% band by operator decision 2026-05-20: flat fixture is degenerate, 40–60% is not meaningful in that context.)* This is the integration test that proves the backtester is honest.
